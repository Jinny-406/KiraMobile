...
async function _integrations(tui) {
  while (true) {
    ...
    else if (choice === 1) tui.addMessage('system', 'use /whatsapp command');
    else if (choice === 2) tui.addMessage('system', 'use /discord command');
    else if (choice === 3) await _proactiveSettings(tui);
...
async function _aboutKira(tui) {
  ...
  console.log(chalk.hex('#f4a7b9')([
    '  i\'m kira.',
    '  i run on your system. i use real tools.',
    ...
    '  /memory      — stored facts',
    '  /workspace   — docs',
    '  /whatsapp    — manage whatsapp settings',
    '  /discord     — manage discord settings',
...
async function _deviceInfo(tui) {
  ...
  info.push(`cores   : ${os.cpus().length}`);
  info.push(`memory  : ${Math.round(os.totalmem() / 1024 / 1024 / 1024)}GB`);
