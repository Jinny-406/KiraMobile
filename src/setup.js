...
async function run() {
  ...
  await sleep(200);
  await printSlowly(`  i run on your system. i use real tools. i learn.`, C.dim, 20);
  await sleep(200);
  ...
  // Detect device
  const platform = os_module.platform();
  const deviceMap = {
    'win32': 'Windows PC',
    'darwin': 'macOS',
    'linux': 'Linux',
  ...
  let device = deviceMap[platform] || platform;

  console.log(chalk.hex(C.hint)(`  device   : ${device}`));
  console.log(chalk.hex(C.hint)(`  platform : ${platform}\n`));
