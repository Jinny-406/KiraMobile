...
function onMessage(name, fn) { messageHandlers.set(name, fn); }
function onToolCall(name, fn) { toolCallHandlers.set(name, fn); }
function onStart(name, fn) { startHandlers.set(name, fn); }
function onStop(name, fn) { stopHandlers.set(name, fn); }
function broadcast(event, data) {
  if (event === 'message') {
    for (const [name, fn] of messageHandlers) { try { fn(data); } catch {} }
  } else if (event === 'toolcall') {
    for (const [name, fn] of toolCallHandlers) { try { fn(data); } catch {} }
  } else if (event === 'start') {
    for (const [name, fn] of startHandlers) { try { fn(data); } catch {} }
  } else if (event === 'stop') {
    for (const [name, fn] of stopHandlers) { try { fn(data); } catch {} }
  }
}

module.exports = { register, execute, list, listWithDescriptions, onMessage, onToolCall, onStart, onStop, broadcast };
