...
// read system state and return structured sensor data
function read() {
  const result = {
    battery:    null,
    temp:       null,
  ...
  // battery — skip on Windows (slow), estimate from hour instead
  // wifi
  try {
    const interfaces = os.networkInterfaces();
    const activeIface = Object.values(interfaces).find(addrs => 
      addrs.some(addr => addr.family === 'IPv4' && !addr.internal)
    );
    result.wifi = activeIface ? 'connected' : null;
  } catch {}
