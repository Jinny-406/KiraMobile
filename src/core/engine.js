...
class Engine {
  ...
  init(soul) { 
    ...
    this.history = persistence.loadHistory();
  }

  async chat(userMessage) {
    const cfg = config.load();
    if (!cfg.apiKey) throw new Error('no API key set. run /config');
    const system = this.soul ? this.soul.buildSystemPrompt() : 'You are Kira.';
    this.history.push({ role: 'user', content: userMessage });
    persistence.appendMessage('user', userMessage);
    ...
    persistence.appendMessage('assistant', reply);
    return reply;
  }
