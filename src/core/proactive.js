...
const PROACTIVE_PROMPT = [
  ...
  '',
  'Do this in order:',
  '1. Check system state: use available system tools',
  '2. Check workspace memory: memory list',
  '3. Review your active goals: list_goals',
  '4. Check reminders: any due soon?',
  '5. Check notifications: weather or news updates?',
  '6. Check health: time for a break?',
  '7. Check social: any auto-posts needed?',
  '8. Check patterns: any learned behaviors to trigger?',
  '',
  ...
  '- Due reminders or health prompts',
...
function start() {
  ...
  if (proactive.reminders) {
    // Reminders are added via commands, but could add default
  }
  if (proactive.notifications.weather) {
    scheduler.addJob({
...
function runCustomScript(script) {
  try {
    // Basic eval with safety (placeholder)
    eval(script);
  } catch (e) {
...
module.exports = { start: start, stop: stop, isAllowed: isAllowed, runCustomScript: runCustomScript };
