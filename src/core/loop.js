...
class AgentLoop {
  async run(userMessage, onThink, onTool, onReply) {
    ...
    while (tools.length > 0 && iter < MAX_ITER) {
      ...
      // Pass tool results as the user message to engine.chat, which adds it to history correctly
      onThink && onThink();
      raw   = await engine.chat(`[tool results]\n${toolResults}\nrespond to the user now.`);

      // No need to remove tool results from history; keep context for future iterations

      tools = parseTools(raw);
      reply = cleanReply(raw);

      // If we have a reply and no more tools, exit early
    ...
    if (iter >= MAX_ITER && !reply) {
      reply = "Hit the iteration limit — I got stuck in a tool loop. Try again, maybe simplify your request.";
    }
